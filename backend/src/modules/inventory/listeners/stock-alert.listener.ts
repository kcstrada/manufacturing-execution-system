...
export class StockAlertListener {
  ...
  async handleAlertCreated(event: StockAlertEvent) {
    this.logger.log(`Processing new stock alert for product: ${event.product.sku}`);
    ...
    try {
      ...
      if (event.alert.severity === 'critical' || event.alert.severity === 'high') {
  ...
  private async sendEmailNotification(event: StockAlertEvent) {
    ...
    this.logger.log(`Email notification queued for ${emailData.to.length} recipients`);
  }
  ...
  private async sendWebhookNotification(event: StockAlertEvent) {
    ...
    await this.notificationQueue.add('webhook-notification', {
      url: webhookUrl,
      method: 'POST',
      headers: {
