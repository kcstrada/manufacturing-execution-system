...
export class InventoryForecastingService {
  ...
  async generateForecast(forecastDto: ForecastDto): Promise<ForecastResultDto> {
    ...
    for (const productId of productIds) {
      const productHistory = historicalData.filter(d => d.productId === productId);
      ...
      const currentInventory = await this.getCurrentInventory(productId, warehouseCode);
      ...
      switch (method) {
        case 'moving_average':
          forecastedDemand = this.calculateMovingAverage(productHistory, forecastDays);
          confidence = this.calculateConfidence(productHistory, forecastedDemand, confidenceLevel);
          break;
        case 'exponential_smoothing':
          forecastedDemand = this.calculateExponentialSmoothing(productHistory, forecastDays);
          confidence = this.calculateConfidence(productHistory, forecastedDemand, confidenceLevel);
          break;
        case 'linear_regression':
          forecastedDemand = this.calculateLinearRegression(productHistory, forecastDays);
          confidence = this.calculateConfidence(productHistory, forecastedDemand, confidenceLevel);
          break;
        default:
          forecastedDemand = this.calculateMovingAverage(productHistory, forecastDays);
      ...
      if (includeSeasonality) {
        const seasonalFactor = this.calculateSeasonalFactor(productHistory, new Date());
    ...
    const totalForecastedDemand = forecastItems.reduce((sum, item) => sum + item.forecastedDemand, 0);
    const averageConfidence = forecastItems.reduce((sum, item) => sum + item.confidence, 0) / forecastItems.length;
    const highRiskItems = forecastItems.filter(item => item.stockoutRisk === 'high').length;
    ...
    return {
      ...
      summary: {
        ...
        averageConfidence,
        highRiskItems,
        criticalItems: forecastItems.filter(item => item.daysUntilStockout !== null && item.daysUntilStockout <= 7).length,
      },
  ...
  async analyzeDemandPatterns(
    ...
    if (transactions.length === 0) {
      throw new NotFoundException(`No demand history found for product ${productId}`);
    }
    ...
    const averageDemand = quantities.reduce((sum, q) => sum + q, 0) / quantities.length;
    const standardDeviation = this.calculateStandardDeviation(quantities, averageDemand);
  ...
  async calculateOptimalReorderPoints(
    ...
    for (const productId of productIds) {
      ...
      const historicalDemand = await this.getHistoricalDemand([productId], undefined, 90);
      ...
      const dailyQuantities = this.aggregateDailyDemand(
        historicalDemand.map(h => ({
          quantity: h.quantity,
          transactionDate: h.date,
        } as any))
      );
      
      const averageDailyDemand = dailyQuantities.reduce((sum, d) => sum + d.quantity, 0) / dailyQuantities.length;
  ...
  async predictStockouts(
    ...
    const inventoryQuery = this.inventoryRepository
      ...
      .andWhere('inventory.status = :status', { status: InventoryStatus.AVAILABLE });

    if (warehouseCode) {
      inventoryQuery.andWhere('inventory.warehouseCode = :warehouseCode', { warehouseCode });
    }
    ...
    for (const inventory of inventoryItems) {
      ...
      // Calculate average daily demand
      const totalDemand = historicalDemand.reduce((sum, d) => sum + d.quantity, 0);
      ...
      const expectedReplenishment = pendingOrders.reduce((sum, order) => sum + order.quantity, 0);
      ...
      predictions.push({
        ...
        projectedDemand,
        daysUntilStockout,
        stockoutDate: daysUntilStockout !== null 
          ? new Date(Date.now() + daysUntilStockout * 24 * 60 * 60 * 1000)
          : null,
        risk,
        probability,
        expectedReplenishment,
        recommendedAction: this.getRecommendedAction(risk, currentStock, averageDailyDemand),
      });
    }
  ...
  private async getHistoricalDemand(
    ...
    const orders = await this.orderRepository
      ...
      .andWhere('order.status NOT IN (:...excludedStatuses)', {
        excludedStatuses: [CustomerOrderStatus.CANCELLED, CustomerOrderStatus.DRAFT],
      })
      .getMany();
  ...
  private async getCurrentInventory(
    ...
    if (warehouseCode) {
      query.andWhere('inventory.warehouseCode = :warehouseCode', { warehouseCode });
    }
  ...
  private calculateMovingAverage(
    ...
    // Calculate average daily demand
    const averageDailyDemand = dailyTotals.reduce((sum, d) => sum + d.quantity, 0) / dailyTotals.length;
  ...
  private calculateExponentialSmoothing(
    ...
    for (let i = 1; i < dailyTotals.length; i++) {
      forecast = alpha * (dailyTotals[i]?.quantity || 0) + (1 - alpha) * forecast;
    }
  ...
  private predictStockout(
    ...
    forecastedDemand: number,
    forecastDays: number,
  ): { daysUntilStockout: number | null; risk: 'low' | 'medium' | 'high' | 'critical' } {
  ...
  private generateRecommendations(
    ...
    if (currentStock < reorderPoint) {
      recommendations.push(`Immediate reorder required - current stock (${currentStock}) is below reorder point (${reorderPoint})`);
    }
    
    if (stockoutPrediction.risk === 'critical') {
      recommendations.push('Critical stockout risk - expedite orders or find alternative suppliers');
    ...
    if (forecastedDemand > currentStock * 2) {
      recommendations.push('Forecasted demand significantly exceeds current stock - consider increasing safety stock levels');
    }
    
    if (stockoutPrediction.daysUntilStockout !== null && stockoutPrediction.daysUntilStockout < 7) {
      recommendations.push(`Stock will be depleted in ${stockoutPrediction.daysUntilStockout} days - urgent action required`);
    }
  ...
  private aggregateDailyDemand(data: any[]): { date: Date; quantity: number }[] {
    const dailyMap = new Map<string, number>();
    
    data.forEach(d => {
      const dateKey = d.date ? d.date.toISOString().split('T')[0] : 
  ...
  private detectTrend(dailyDemand: { date: Date; quantity: number }[]): boolean {
  ...
  private detectSeasonality(dailyDemand: { date: Date; quantity: number }[]): boolean {
    ...
    dayOfWeekDemand.forEach(quantities => {
      if (quantities.length > 0) {
        dayAverages.push(quantities.reduce((sum, q) => sum + q, 0) / quantities.length);
      }
    });
    
    if (dayAverages.length < 3) return false;
    
    const overallAvg = dayAverages.reduce((sum, a) => sum + a, 0) / dayAverages.length;
  ...
  ): { direction: 'increasing' | 'decreasing' | 'stable'; strength: number; growthRate: number } {
  ...
  private identifyPeakPeriods(
    ...
    dailyDemand.forEach(d => {
      ...
      dayOfWeekDemand.set(dayOfWeek, (dayOfWeekDemand.get(dayOfWeek) || 0) + d.quantity);
    });
  ...
  private calculateForecastAccuracy(dailyDemand: { date: Date; quantity: number }[]): number {
  ...
  private async getPendingOrdersForProduct(
    ...
    return orders.map(order => ({
      quantity: order.orderLines?.find(l => l.productId === productId)?.quantity || 0,
