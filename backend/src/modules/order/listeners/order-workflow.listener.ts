...
export class OrderWorkflowListener {
  ...
  private async handleOrderConfirmed(
    ...
    try {
      ...
      if (result.warnings.length > 0) {
        this.logger.warn(
          `Warnings during task generation: ${result.warnings.join(', ')}`,
        );
      }
    } catch (error) {
      this.logger.error(
  ...
  private async handleProductionStarted(
    event: OrderStateChangedEvent,
  ): Promise<void> {
  ...
  private async handleQualityControlStarted(
    event: OrderStateChangedEvent,
  ): Promise<void> {
  ...
  private async handleQualityControlPassed(
    event: OrderStateChangedEvent,
  ): Promise<void> {
  ...
  private async handleQualityControlFailed(
    event: OrderStateChangedEvent,
  ): Promise<void> {
  ...
  private async handleOrderOnHold(
    event: OrderStateChangedEvent,
  ): Promise<void> {
  ...
  async handleProductionComplete(payload: {
    orderId: string;
    orderNumber: string;
  }): Promise<void> {
  ...
  async handleQCComplete(payload: {
    ...
    if (payload.passed) {
      ...
      this.logger.warn(
        `QC failed for order ${payload.orderNumber}: ${payload.issues?.join(', ')}`,
      );
    }
  }
  ...
  async handlePaymentReceived(payload: {
    ...
    this.logger.log(
      `Payment received for order ${payload.orderId}: $${payload.amount}`,
    );
  ...
  async handleInventoryAllocated(payload: {
    ...
    if (payload.success) {
      ...
      this.logger.warn(
        `Failed to allocate inventory for order ${payload.orderId}`,
      );
    }
  }
}
