...
export class AddProductManufacturingFields1758036169186
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "products" ADD "is_manufacturable" boolean NOT NULL DEFAULT true`,
    );
    await queryRunner.query(
      `ALTER TABLE "products" ADD "is_purchasable" boolean NOT NULL DEFAULT false`,
    );
    await queryRunner.query(
      `ALTER TABLE "products" ADD "barcode" character varying(100)`,
    );
    await queryRunner.query(`ALTER TABLE "products" ADD "default_bom_id" uuid`);
    await queryRunner.query(
      `ALTER TABLE "products" ADD "default_routing_id" uuid`,
    );

    // Create indexes for better query performance
    await queryRunner.query(
      `CREATE INDEX "idx_products_tenant_id_barcode" ON "products" ("tenant_id", "barcode")`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_products_tenant_id_is_manufacturable" ON "products" ("tenant_id", "is_manufacturable")`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_products_tenant_id_is_purchasable" ON "products" ("tenant_id", "is_purchasable")`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Drop indexes
    await queryRunner.query(
      `DROP INDEX "public"."idx_products_tenant_id_is_purchasable"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."idx_products_tenant_id_is_manufacturable"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."idx_products_tenant_id_barcode"`,
    );

    // Drop columns
    await queryRunner.query(
      `ALTER TABLE "products" DROP COLUMN "default_routing_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "products" DROP COLUMN "default_bom_id"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "products" DROP COLUMN "is_manufacturable"`,
    );
  }
}
