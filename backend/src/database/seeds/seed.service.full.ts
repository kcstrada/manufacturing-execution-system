  ...
  async seed() {
    ...
    try {
      ...
      await queryRunner.query(`
        ...
        ($1, $6, $7, $2, 1, true),
        ($1, $8, $9, $4, 1, true)
      `, [tenant1.id, users[0].id, roles[0].id, users[1].id, roles[1].id, 
          users[2].id, roles[2].id, users[3].id, roles[3].id]);
      ...
      const products = await queryRunner.query(`
        ...
        RETURNING id
      `, [tenant1.id, categories[0].id, uoms[0].id, categories[1].id, 
          categories[2].id, uoms[1].id, categories[3].id]);
